...
export function PlayersTable({
  ...
  data: TPlayer[];
  onSelectingAPlayer: (player: TPlayer) => void;
  columns: string[];
}) {
  const pageSize = 10;
          ...
          <div>
            <div className="shadow-lg overflow-hidden">
              <table className="table text-gray-400 border-separate border-spacing-x-0 border-spacing-y-0.5 text-sm">
                <thead className="bg-gradient-to-r from-tableGradientFrom to-tableGradientTo text-white">
                  <tr>
                ...
                <tbody>
                  {paginatedData.map((item, index) => (
                    <tr key={index} className="bg-gradient-to-r from-tableGradientFrom to-tableGradientTo text-white cursor-pointer">
    ...
    <div className="overflow-x-auto">
      ...
      {playersData.length === 0 && (
        <div className="flex justify-center w-[100%]">
          <p className="text-[26px] text-white font-bold my-[50px]]">
            No players found with the specified name
          </p>
        </div>
      )}
    </div>
